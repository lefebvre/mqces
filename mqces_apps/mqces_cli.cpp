...
//       "epsilon":           0.05,                    // default 0
//       "mc_samples":        10,                      // default 10
//       "seed":              42,
//       "nota_threshold":    0.05,                    // default 0.05
//       "nota_permutations": 199,                     // default 199
//       "n_threads":         0,                       // default 0 (auto)
//       "variant":           "classic" | "v2"         // default "classic"
//     }
//   }
//
//   epsilon            measurement-error fraction; 0 omits that term
//   mc_samples         perturbation replicates, >= 2; used only when epsilon > 0
//   nota_threshold     p-value threshold in (0, 1)
//   nota_permutations  none-of-the-above permutations; 0 skips the test
//
...
mqces::Sample parse_sample(const json& j, const char* what) {
  ...
  for (Eigen::Index i = 0; i < n; ++i) {
    ...
    for (Eigen::Index k = 0; k < d; ++k) {
      s(i, k) = j[static_cast<std::size_t>(i)][static_cast<std::size_t>(k)].get<double>();
    }
  }
  return s;
}
...
ParsedInput parse_input(const json& j) {
  ...
  in.options.uncertainty.epsilon = opt.value("epsilon", in.options.uncertainty.epsilon);
  in.options.uncertainty.mc_samples = opt.value("mc_samples", in.options.uncertainty.mc_samples);
  in.options.uncertainty.seed = opt.value("seed", in.options.uncertainty.seed);
  in.options.nota_threshold = opt.value("nota_threshold", in.options.nota_threshold);
  in.options.nota_permutations = opt.value("nota_permutations", in.options.nota_permutations);
  ...
  if (in.variant != "classic" && in.variant != "v2") {
    throw std::runtime_error(R"(options.variant: must be "classic" or "v2" (got ")" + in.variant +
                             "\")");
  }
  return in;
}
...
int run(int argc, char** argv) {
  ...
  try {
    ...
    std::cerr << "error: failed to parse JSON from " << input_path << ": " << e.what() << "\n";
    return 1;
  }
