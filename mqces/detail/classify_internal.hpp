    ...
    const Sample& test, std::span<const Class> known, const ClassifierOptions& opts)
{
    ...
    for (const auto& cls : known) {
        if (cls.specimens.cols() != d) {
            throw std::invalid_argument(
                "classify: class '" + cls.name + "' has feature count "
                + std::to_string(cls.specimens.cols())
                + " but test has " + std::to_string(d));
        }
        if (cls.specimens.rows() < 2) {
            throw std::invalid_argument(
                "classify: class '" + cls.name + "' needs at least 2 specimens");
        }
    }
    ...
    if (opts.nota_permutations > 0
        && static_cast<double>(opts.nota_permutations + 1) * alpha < 1.0) {
        throw std::invalid_argument(
...
inline Eigen::VectorXd observed_scores(
    const Sample& test, std::span<const Class> known, const ClassifierOptions& opts)
{
    Eigen::VectorXd scores(static_cast<Eigen::Index>(known.size()));
    parallel_for(known.size(), opts.n_threads, [&](std::size_t k) {
        scores(static_cast<Eigen::Index>(k))
            = similarity_score(test, known[k].specimens, opts.weights);
    });
    return scores;
}
...
inline Replicates collect_replicates(
    const Sample& test, const Class& cls, std::size_t class_index,
    const ClassifierOptions& opts)
{
    const Eigen::Index n_test  = test.rows();
    const Eigen::Index n_class = cls.specimens.rows();
    const double       eps     = opts.uncertainty.epsilon;
    const Eigen::Index n_mc
        = eps > 0.0 ? static_cast<Eigen::Index>(opts.uncertainty.mc_samples) : 0;
    ...
    parallel_for(static_cast<std::size_t>(n_tasks), opts.n_threads, [&](std::size_t task) {
        ...
        if (t < n_test) {
            rep.drop_test(t)
                = similarity_score(drop_row(test, t), cls.specimens, opts.weights);
        } else if (t < n_test + n_class) {
            const Eigen::Index j = t - n_test;
            rep.drop_class(j)
                = similarity_score(test, drop_row(cls.specimens, j), opts.weights);
        } else {
            const Eigen::Index r  = t - n_test - n_class;
            const auto         ri = static_cast<std::size_t>(r);
            const auto         base = opts.uncertainty.seed;
            rep.perturbed(r)        = similarity_score(
                perturb(test, eps, replicate_seed(base, ri, static_cast<std::size_t>(-1))),
