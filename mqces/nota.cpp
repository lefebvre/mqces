    ...
    const ClassifierOptions& options)
{
    ...
    if (test.cols() != best.specimens.cols()) {
        throw std::invalid_argument(
            "is_none_of_the_above: test and class '" + best.name
    ...
    const auto extreme
        = std::count(at_least_observed.begin(), at_least_observed.end(), char{1});
    const double p_value
        = static_cast<double>(1 + extreme) / static_cast<double>(1 + n_perm);
