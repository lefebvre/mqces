...
std::vector<mqces::Class> to_classes(
  ...
  for (const auto& [name, arr] : py_classes) {
    if (arr.ndim() != 2) {
      throw std::invalid_argument("class '" + name + "' specimens must be a 2-D array");
    }
...
mqces::Sample to_sample(const nb::ndarray<double, nb::c_contig>& arr, const char* what) {
  if (arr.ndim() != 2) {
...
mqces::ClassifierOptions build_options(const nb::ndarray<double, nb::c_contig>& weights_arr,
                                       Eigen::Index expected_d,
                                       double epsilon,
...
nb::dict classify_wrapper(
  ...
  auto opts = build_options(weights_arr,
                            test.cols(),
                            epsilon,
                            mc_samples,
                            seed,
