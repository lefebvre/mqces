...
TEST(ClassifyClassic, ThrowsOnSingleSpecimenClass)
{
    ...
    EXPECT_THROW(mqces::classic::classify(test, std::span<const Class>{known},
                                          default_options(3)),
                 std::invalid_argument);
}
...
TEST(ClassifyClassic, MeasurementErrorWidensUncertainty)
{
    ...
    const auto p0 = mqces::classic::classify(test, std::span<const Class>{known}, opts)
                        .misclassification_prob;
    opts.uncertainty.epsilon = 0.2;
    const auto p1 = mqces::classic::classify(test, std::span<const Class>{known}, opts)
                        .misclassification_prob;
    EXPECT_GT(p1, p0);
}
