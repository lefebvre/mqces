...
// v3 must agree with v2 on which class is best — the inner solver does
// not change which mean score is lowest in expectation, only how
// numerically tight the per-replicate scores are.
TEST(ClassifyV3, BestClassAgreesWithV2)
{
    constexpr int d = 3;
    ...
    EXPECT_EQ(r_v2.best_class, r_v3.best_class);
    EXPECT_EQ(r_v3.best_class, "B");
}

// Misclass probability is in [0, 1].
TEST(ClassifyV3, MisclassProbInUnitInterval)
{
    constexpr int d = 2;
    std::vector<Class> known{
...
TEST(ClassifyV3, IsDeterministic)
{
    ...
    auto   opts = default_options(d);
    auto   r1   = mqces::v3::classify(test, std::span<const Class>{known}, opts);
    auto   r2   = mqces::v3::classify(test, std::span<const Class>{known}, opts);
    EXPECT_EQ(r1.best_class, r2.best_class);
    EXPECT_EQ(r1.misclassification_prob, r2.misclassification_prob);
}

// When the caller passes a SolverKind::Weiszfeld override, v3 should still
// honor it (the SolverConfig is plumbed through; v3's only requirement is
// that *if* the kind is left as default, VZ is selected internally).
TEST(ClassifyV3, RespectsExplicitSolverOverride)
{
    constexpr int d = 3;
    std::vector<Class> known{
    ...
    auto opts          = default_options(d);
    opts.solver.kind   = mqces::SolverKind::VardiZhang;
    opts.solver.tol    = 1e-8;
    opts.solver.max_iters = 300;

    auto res = mqces::v3::classify(test, std::span<const Class>{known}, opts);
