...
TEST(PaperTable1, ScoreMatrixMatchesReference) {
  ...
  for (int k = 0; k < tf::N_CLASSES; ++k) {
    ...
    for (int j = 0; j < tf::N_CLASSES; ++j) {
      const double* class_base = tf::class_data + j * tf::N_SPECIMENS * tf::N_FEATURES;
      ...
      if (!ok) {
        ...
        EXPECT_TRUE(ok) << "S(test=" << k << ", class=" << j << "): "
                        << "cpp=" << cpp_score << ", ref=" << ref_score << ", abs=" << abs_diff
                        << ", rel=" << rel_diff;
      }
      ++compared;
    }
...
TEST(PaperTable1, ReferenceScoresAreOrdered) {
  for (int k = 0; k < tf::N_CLASSES; ++k) {
    ...
    const int j_far = (k < tf::N_CLASSES / 2) ? (tf::N_CLASSES - 1) : 0;
    const double s_kfar = tf::expected_scores[k * tf::N_CLASSES + j_far];
    EXPECT_LT(s_kk, s_kfar) << "Reference: S(test_" << k << ", class_" << k << ") should be < "
